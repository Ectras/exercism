...
def check_drinks(drink_name: str, drink_ingredients: list[str]) -> str:
    ...
    """
    ending = (
        "Cocktail" if len(ALCOHOLS.intersection(drink_ingredients)) > 0 else "Mocktail"
    )
    return f"{drink_name} {ending}"
...
def compile_ingredients(dishes: list[set[str]]) -> set[str]:
    ...
    """
    return functools.reduce(lambda acc, set: acc | set, dishes)
